...
void contextmenu_render(void) {
    ...
    for (int i = 0; i < g_ctx_item_count; i++) {
        ...
        // Item label, vertically centered within the item row.
        int32_t text_y = item_y + (CTX_MENU_ITEM_H - FONT_CHAR_H) / 2;
        draw_text(mx + 8, text_y, g_ctx_items[i].label, CLR_TEXT_WHITE);
    }
}
    ...
    if (clicked && hovered >= 0) {
        ...
        switch (action) {
            case CTX_ACTION_WALLPAPER:
                wallpaper_picker_open();
                break;

            // All other actions are intentionally no-ops for now.
