...
static int64_t console_write(file_t *f, const void *buf, size_t count) {
    ...
    // #19/#645: `p` is the caller's buffer; every process has /dev/console on
    // fds 0/1/2, so this is the write path for ordinary printf output. kputc()
    // touches only kernel state, so keeping it inside the window costs nothing
    // and avoids one stac/clac pair per byte.
    ...
    if (count < 256) {
        ...
        // #19/#645: the second, separate read of the caller's buffer.
        {   uaccess_ac_t __ac = uaccess_begin();
...
static int console_release(file_t *f) {
    (void)f;
    // No per-file state to free; the file_t itself is kfree()'d by file_put.
    // #695: nothing is buffered here, so 0 is a fact, not a stub.
    return 0;
}
