...
// MayteraOS port: exceptions disabled; fatal paths abort via curaslice_die.
extern "C" void curaslice_die(const char*);
  ...
  public:
    ...
    Int128 operator/ (const Int128 &rhs) const
    {
      if (rhs.lo == 0 && rhs.hi == 0)
        curaslice_die("Int128 operator/: divide by zero");
...
{
  if (p1 == p2) curaslice_die("JoinError");
...
void RangeTest(const IntPoint& Pt, bool& useFullRange)
{
  if (useFullRange)
  {
    if (Pt.X > hiRange || Pt.Y > hiRange || -Pt.X > hiRange || -Pt.Y > hiRange) 
      curaslice_die("Coordinate outside allowed range");
  }
...
bool ClipperBase::AddPath(const Path &pg, PolyType PolyTyp, bool Closed)
{
  ...
  if (!Closed && PolyTyp == ptClip)
    curaslice_die("AddPath: Open paths must be subject.");
#else
  if (!Closed)
    curaslice_die("AddPath: Open paths have been disabled.");
#endif
  ...
  //MayteraOS port: try/catch removed (-fno-exceptions); RangeTest failures
  //abort via curaslice_die inside RangeTest, so the catch was unreachable.
  {
    edges[1].Curr = pg[1];
    ...
    PolyFillType subjFillType, PolyFillType clipFillType)
{
  ...
  if (m_HasOpenPaths)
    curaslice_die("Error: PolyTree struct is need for open path clipping.");
...
{
  bool succeeded = true;
  //MayteraOS port: try/catch removed (-fno-exceptions); the only throwers in
  //this subset now abort via curaslice_die, so the failure path is unreachable.
  {
    Reset();
     ...
     const IntPoint &Pt, bool protect)
{
  ...
  if (e1->WindDelta == 0 || e2->WindDelta == 0)
  {
    ...
    if (e1stops)
      ...
      else curaslice_die("Error intersecting polylines");
    if (e2stops) 
...
void Clipper::ProcessHorizontal(TEdge *horzEdge, bool isTopOfScanbeam)
{
    ...
    while(e)
    {
        ...
        (dir == dRightToLeft && e->Curr.X >= horzLeft))
      {
        ...
        if(e == eMaxPair && IsLastHorz)
        {
          ...
          if (eMaxPair->OutIdx >= 0) curaslice_die("ProcessHorizontal error");
          return;
        }
  ...
  else if (eMaxPair)
  {
    if (eMaxPair->OutIdx >= 0)
    {
      ...
      if (eMaxPair->OutIdx >= 0)
        curaslice_die("ProcessHorizontal error");
    } else
    {
...
{
  if( !e->NextInLML ) curaslice_die("UpdateEdgeIntoAEL: invalid call");
...
bool Clipper::ProcessIntersections(const cInt botY, const cInt topY)
{
  ...
  //MayteraOS port: try/catch removed (-fno-exceptions); see note above.
  {
    ...
    while( e->NextInSEL )
    {
      ...
      if(e->Curr.X > eNext->Curr.X)
      {
        if (!IntersectPoint(*e, *eNext, Pt, m_UseFullRange) && e->Curr.X > eNext->Curr.X +1)
          curaslice_die("Intersection error");
        if (Pt.Y > botY)
        {
            Pt.Y = botY;
...
#endif
  else curaslice_die("DoMaxima error");
}
//------------------------------------------------------------------------------
...
// MayteraOS port: bodies stubbed. These debug printers are never called in
// the slice pipeline; keeping the << chains would pull libstdc++ iostream
// (ostream::_M_insert / __ostream_insert) into a -nostdlib link. Signatures
// kept so the header declarations still resolve.
