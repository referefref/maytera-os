...
#include "../platform.h"   // MayteraOS: pstring (mstring) replaces std::string
...
int main(int argc, char **argv)
{
    ...
    // MayteraOS: apply the built-in slicing preset (presets.h). The upstream
    // config file path is stubbed out on this platform, so the profile is
    // compiled in and applied here before any -s overrides on the command line.
    for (unsigned pi = 0; pi < CURASLICE_PRESET_DEFAULT_COUNT; pi++) {
        char kv[128];
        int j = 0;
        const char *e = CURASLICE_PRESET_DEFAULT[pi];
        while (e[j] && j < (int)sizeof(kv) - 1) { kv[j] = e[j]; j++; }
        kv[j] = '\0';
        char *vp = strchr(kv, '=');
        if (vp) { *vp++ = '\0'; config.setSetting(kv, vp); }
    }

    fffProcessor processor(config);
    std::vector<pstring> files;

    // MayteraOS self-test: when launched with no arguments (for example via the
    // kernel AUTORUN hook, which passes none), slice a bundled cube to a fixed
    // output so the port can be smoke-tested on-device without an arg-passing
    // launcher. Real invocations from the 3D Print app pass argv normally.
    static char *self_argv[] = { (char*)"curaslice", (char*)"-v", (char*)"-p",
                                 (char*)"-o", (char*)"/OUT.GCODE",
                                 (char*)"/CUBE.STL", 0 };
    if (argc < 2) { argc = 6; argv = self_argv; }
        ...
        if (str[0] == '-')
        {
            for(str++; *str; str++)
            {
                ...
                case 'm':
                    ...
                    break;
                case '-':
                    // MayteraOS: built -fno-exceptions, so the try/catch guard is
                    // gone. ClipperLib's internal-error throws are compiled out;
                    // on such an error the __throw_* stubs in cxxsupp.cpp abort.
                    if (files.size() > 0)
            ...
            if (argv[argn][0] == '$')
            {
                // MayteraOS: -fno-exceptions, no try/catch guard (see above).
                std::vector<pstring> tmp;
    ...
    // MayteraOS on-device breadcrumbs: serial capture is unreliable in the test
    // harness, so record how far the slice got to a file the host reads back
    // after a clean shutdown. Harmless on real hardware (a tiny log file).
    FILE *cl = fopen("/CURA.LOG", "w");
    if (cl) { fprintf(cl, "start files=%d\n", (int)files.size());
              if (files.size() > 0) fprintf(cl, "input=%s\n", files[0].c_str());
              fflush(cl); }
    // MayteraOS: -fno-exceptions, no try/catch guard (see above).
    if (files.size() > 0)
        processor.processFile(files);
    if (cl) { fprintf(cl, "processFile returned\n"); fflush(cl); }
    //Finalize the processor, this adds the end.gcode. And reports statistics.
    processor.finalize();
    if (cl) {
        fprintf(cl, "finalized\n");
        FILE *g = fopen("/OUT.GCODE", "r");
        if (g) { fseek(g, 0, SEEK_END); long sz = ftell(g); fclose(g);
                 fprintf(cl, "gcode bytes=%ld\n", sz); }
        else    fprintf(cl, "gcode OPEN FAILED\n");
        fprintf(cl, "done\n"); fflush(cl); fclose(cl);
    }
    return 0;
}
