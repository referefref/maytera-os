#include <cctype>
// MayteraOS: <fstream> removed. std::ifstream/std::getline pull in libstdc++
// iostream/locale static init, which cannot link on the -nostdlib userland.
// Config-file reading is disabled here; settings arrive via -s key=value and the
// built-in preset (presets.h). See CURAENGINE_PORT_PLAN.md section 3.4/3.5.
#include <stdio.h>
...
bool ConfigSettings::readSettings(const char* path) {
    // MayteraOS: config-file reading is disabled. The original body used
    // std::ifstream + std::getline (<fstream>), which drags libstdc++
    // iostream/locale static init that cannot link on the -nostdlib userland.
    // Settings are supplied programmatically via setSetting() from the built-in
    // preset (presets.h) and from -s key=value CLI arguments. Report "not read"
    // so callers fall back to the constructor defaults, which is the same
    // contract main.cpp already handles for a missing default.cfg.
    (void)path;
    return false;
}
