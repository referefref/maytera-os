// glmatrix - 3D "matrix code rain" (TinyGL) for MayteraOS. Renders the shared
// gldemo matrix core into an ARGB buffer and pushes it with SYS_WIN_BLIT. (#319)
...
int main(int argc, char *argv[]) {
    (void)argc; (void)argv;
    int win = win_create("GL Matrix", 100, 80, 760, 580);
    if (win < 0) return 1;
    int cw = 740, ch = 560;
    if (win_get_size(win, &cw, &ch) != 0 || cw <= 0 || ch <= 0) { cw = 740; ch = 560; }
    if (cw > MAXW) cw = MAXW; if (ch > MAXH) ch = MAXH;
    gldemo_init(GLDEMO_MATRIX, cw, ch);
