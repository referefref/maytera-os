...
int64_t syscall_dispatch(uint64_t num, uint64_t arg1, uint64_t arg2,
    ...
    switch (num) {
        ...
        case SYS_NET_DHCP: { extern int dhcp_discover_blocking(void); return (int64_t)dhcp_discover_blocking(); }
...
int64_t sys_open(const char *path, int flags) {
    ...
    if (path_is_smb(path)) {
        ...
        if (have && info.is_directory) {
            ...
            return -MOS_ENOENT;  // not found and not creating
        }
        s->used = 1;
        fd_used[fd] = 1;
        return fd;
    ...
    } else if (path_root_ext2(path)) {
        if (ext2_resolve_path(path) != 0 || (flags & 0x40)) rel = path;
    }
    if (rel) {
        ext2_fd_t *e = &e2fd[fd];
...
static void async_post_worker(void *arg) {
    ...
    for (;;) {
        ...
        if (!did) proc_sleep(10);   // idle: yield until a job arrives
    }
}
...
#define DEFAULT_BRK_START   (USER_SPACE_START + 0x100000)
...
static void user_window_draw_handler(void *app_data) {
    ...
    if (handle >= 0 && handle < MAX_USER_WINDOWS && user_windows[handle].window) {
        ...
        // Send a redraw event to user-space, but coalesce: skip if one is
        // already queued.  This prevents redraw events from flooding the
        // 128-entry queue and pushing out keyboard/mouse events.
        {
            int has_redraw = 0;
...
int64_t sys_win_get_event(int handle, void *event_buf, int timeout) {
    ...
    // If no events and timeout is 0, return immediately
    if (uw->event_count == 0) {
        if (timeout == 0) {
            return 0;  // No event, non-blocking
        }

        // Wait for event (simple busy-wait with timeout)
        // TODO: Use proper sleep/wake mechanism
        uint64_t start = timer_ticks;
        uint64_t timeout_ticks = (timeout > 0) ? (uint64_t)timeout / 10 : 0xFFFFFFFFFFFFFFFFULL;

        while (uw->event_count == 0) {
            if ((timer_ticks - start) >= timeout_ticks) {
                return 0;  // Timeout, no event
            }
            // Yield to let other processes run and generate events
            proc_yield();
        }
    }
...
#define SANITIZE_USER_PTR(ptr, type) do { \
    ...
    if ((__addr & 0xFFFFFFFF00000000ULL) == 0xFFFFFFFF00000000ULL) { \
...
int64_t sys_ntp_sync(void) {
    ...
    while (!g_ntp_done && timer_ticks < deadline)
        asm volatile("pause");
